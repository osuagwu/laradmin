{{-- Display an Autoform for editing
    INPUT
...
@section('content')
<section role="main" class="section section-full-page section-subtle section-diffuse section-light-bg">
                ...
                @foreach($form->getGroupedFields() as $group_name=> $fields)
                    ...
                    <div class="group">
                        @if($form->getGroup($group_name) and $form->getGroup($group_name)->label)
                            <h6 class="label label-subtle ">{{$form->getGroup($group_name)->label}}</h6>
