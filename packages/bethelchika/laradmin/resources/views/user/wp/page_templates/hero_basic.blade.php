...
<div class="@if(!$metas['hero']['is_fullscreen']) container{{$laradmin->assetManager->isContainerFluid('-fluid')}} @endif">{{-- XCC:We wrap the section with container class (and remove it below) if we want to constrain the width of the hero--}}
    <section class="section section-primary hero   @if(str_contains($page->meta->hero_height,'full')) section-full-height @else section-extra-padding-bottom @endif" style="@if(is_numeric($page->meta->hero_height)) min-height: {{intval($page->meta->hero_height)}}vh; @endif">
        <div class="section-overlay ">
                ...
                <div class="hero-content-box  @if($page->meta->hero_content_position)  {{$page->meta->hero_content_position}} @else left @endif">
