    ...
    <article class="blog-post {{$class??''}}" title="{{$post->title}}">
                ...
                <div class="blog-post-inner">
                    <div class="img-box blog-post-inner-item">
                            <a href="@if($post->post_type=='laradmin_larus_post'){{route('larus-post',$post->post_name)}}@else{{route('post',$post->post_name)}}@endif" >
                            <img src="{{$post->getFeaturedThumb('laradmin-thumb',false)??'https://via.placeholder.com/270x150.png?text='.urlencode('Read More')}}" alt="{{$post->title}}">
                        </a>
                    </div>
