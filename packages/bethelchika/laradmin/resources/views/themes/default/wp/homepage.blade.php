...
<!-- <div role="main" class="@if(!$metas['hero']['is_fullscreen']) container{{$laradmin->assetManager->isContainerFluid('-fluid')}} @endif">{{-- XCC:We wrap the section with container class (and remove it below) if we want to constrain the width of the hero--}}
    <section class="section section-{{$post->meta->scheme??'primary'}}  hero {{$laradmin->assetManager->isSuperHero('hero-super')}}  @if(str_contains($post->meta->hero_height,'full')) section-full-height @else section-extra-padding-bottom @endif" >
        <div class="section-overlay section-overlay-gradient-{{$post->meta->hero_shade??'smooth'}}">
                ...
                <div class="hero-content-box    {{$post->meta->hero_headline_justify??'left'}}  {{$post->meta->hero_headline_align??'middle'}} " style="@if(is_numeric($post->meta->hero_height)) min-height: {{intval($post->meta->hero_height)}}vh; @endif">
