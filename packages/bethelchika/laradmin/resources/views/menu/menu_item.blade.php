...
--}} 
  ...
  <li role="{{$item->ariaRole}}" class="menu-item level-{{$level}} {{$item->cssClass }} {{$class ?? ''}}
    ...
    @if($item->hasLink()) 
      <a {!! $item->getHtmlAttributes() !!}
          @if($item->hasChildren()) class="dropdown-toggle" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" @else class="" @endif  
